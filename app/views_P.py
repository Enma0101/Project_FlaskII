...
@app.route('/get_dni_by_estudiante/<int:estudianteId>', methods=['GET'])
def get_dni_by_estudiante(estudianteId):
    conn = get_db()
    cursor = conn.execute('''
    ...
    if estudiante:
        return jsonify({'dni_estudiante': estudiante['dni_estudiante']})
    else:
        return jsonify({'dni_estudiante': None})
